 ...
 */
void
sanctum_mlkem1024_keypair(struct sanctum_mlkem1024 *ctx)
{
	PRECOND(ctx != NULL);

	(void)pqcrystals_kyber1024_ref_keypair(ctx->pk, ctx->sk);
}
 ...
 */
void
sanctum_mlkem1024_encapsulate(struct sanctum_mlkem1024 *ctx)
{
	PRECOND(ctx != NULL);

	(void)pqcrystals_kyber1024_ref_enc(ctx->ct, ctx->ss, ctx->pk);
}
 ...
 */
void
sanctum_mlkem1024_decapsulate(struct sanctum_mlkem1024 *ctx)
{
	PRECOND(ctx != NULL);

	(void)pqcrystals_kyber1024_ref_dec(ctx->ss, ctx->ct, ctx->sk);
}
