...
/* The indicator for -v. */
const char	*sanctum_cipher = "libsodium-aegis";
...
sanctum_cipher_setup(struct sanctum_key *key)
{
	...
	nyfe_memcpy(cipher->key, key->key, sizeof(key->key));
...
sanctum_cipher_encrypt(struct sanctum_cipher *cipher)
{
	...
	if (crypto_aead_aegis256_encrypt_detached(cipher->ct,
	    cipher->tag, NULL, cipher->pt, cipher->data_len, cipher->aad,
	    cipher->aad_len, NULL, cipher->nonce, ctx->key) == -1)
...
sanctum_cipher_decrypt(struct sanctum_cipher *cipher)
{
	...
	if (crypto_aead_aegis256_decrypt_detached(cipher->pt,
	    NULL, cipher->ct, cipher->data_len, cipher->tag, cipher->aad,
	    cipher->aad_len, cipher->nonce, ctx->key) == -1)
		return (-1);
